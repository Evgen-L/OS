    ...
    public class StateConnectiongs
    {
        ...
        public override string ToString()
        {
            ...
            foreach (var connect in Connectiongs.Skip(1)) 
                result += $" | {connect.Key}{new string(connect.Value.ToArray())}";
            return result;
        }
        ...
        //     $"{Connectiongs.FirstOrDefault().Key}{new string(Connectiongs.FirstOrDefault().Value.ToArray())}";
        
        private string ToStringFirstConnect() => 
            $"{Connectiongs.FirstOrDefault().Key}{(Connectiongs.FirstOrDefault().Value == null ? "" : new string(Connectiongs.FirstOrDefault().Value.ToArray()))}";
